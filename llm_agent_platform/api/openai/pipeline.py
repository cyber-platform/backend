...
from llm_agent_platform.core.utils import (
    sanitize_data,
    sanitize_string,
    clean_gemini_schema,
)
...
def _build_gemini_config(data: dict) -> dict:
    ...
    max_thinking_tokens = data.get("modelMaxThinkingTokens") or data.get(
        "max_completion_tokens"
    )
...
def _resolve_group_models(
    provider: ProviderDescriptor, group_id: str | None
...
) -> tuple[ProviderDescriptor, str, tuple[str, ...] | None]:
...
def _resolve_openai_chatgpt_effective_request_params(
    *,
    data: dict,
    target_model: str,
    resolved_group_id: str,
) -> tuple[str | None, dict[str, str]]:
    key_record = getattr(g, "openai_chatgpt_platform_key", None)
    ...
    if not isinstance(key_id, str) or not key_id.strip():
        return None, {}
    if key_record.get("group_id") != resolved_group_id:
        raise UpstreamPreparationError(
            f"Platform API key group does not match resolved group '{resolved_group_id}'",
            "invalid_request_error",
            400,
        )

    policy_service = OpenAIChatGPTRequestPolicyRegistryService()
    policy_record = policy_service.get_policy(key_id)
    if not policy_record.model_overrides:
        return key_id, {}

    OpenAIChatGPTModelCapabilitiesService().get_model_capabilities(target_model)
    model_policy = policy_record.model_overrides.get(target_model)
    if model_policy is None:
        return key_id, {}

    effective_request_params: dict[str, str] = {}
    for parameter_name, override in model_policy.items():
        client_value = data.get(parameter_name)
        if override.mode == "force":
            effective_request_params[parameter_name] = override.value
            continue
        if parameter_name not in data or client_value is None:
            effective_request_params[parameter_name] = override.value
            continue
        if isinstance(client_value, str):
            effective_request_params[parameter_name] = client_value.strip()
            continue
        effective_request_params[parameter_name] = str(client_value)

    return key_id, effective_request_params
...
def build_request_context(
    ...
    if not isinstance(raw_model, str) or not raw_model.strip():
        raise UpstreamPreparationError(
            "Field 'model' is required", "invalid_request_error", 400
        )
    ...
    if provider.provider_id == "openai-chatgpt":
        platform_key_id, effective_request_params = (
            _resolve_openai_chatgpt_effective_request_params(
                data=data,
    ...
    transformed_contents, system_instruction = transform_openai_to_gemini(messages)
    contents: list[Any] = cast(
        list[Any],
        sanitize_data(
            transformed_contents if isinstance(transformed_contents, list) else []
        ),
    )
    ...
    provider_cfg = quota_account_router.try_load_provider_config(
    ...
    is_qwen_quota_mode = (
        uses_account_router and provider.runtime_adapter == QwenCodeProvider.id
    )
    is_gemini_quota_mode = (
        uses_account_router and provider.runtime_adapter == GeminiCliProvider.id
    )
    ...
    user_prompt_id = (
        data.get("user_prompt_id")
        or request.headers.get("x-user-prompt-id")
        or generate_user_prompt_id()
    )

    return ChatRequestContext(
        provider_id=provider.provider_id,
        provider_runtime_adapter=provider.runtime_adapter,
        provider_transport_kind=provider.raw_payload.get("transport", {}).get(
            "kind", ""
        ),
        ...
        effective_request_params=effective_request_params,
    )
...
def build_fallback_chain(ctx: ChatRequestContext) -> list[str]:
    ...
    if (
        not (ctx.is_quota_mode and STRICT_CLI_PARITY)
        and "gemini-3-flash" in ctx.target_model
    ):
        fallback_chain.extend(["gemini-2.5-flash", "gemini-2.5-flash-lite"])
    elif (
        not (ctx.is_quota_mode and STRICT_CLI_PARITY)
        and "gemini-3.1-pro" in ctx.target_model
    ):
